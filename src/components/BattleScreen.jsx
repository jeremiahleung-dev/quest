...
import { SKILL_DEFS } from "../data/gameData.js";

export default function BattleScreen({ dungeon, character, onEnd, dark }) {
  ...
  const [effects, setEffects]         = useState({ guard: 0, shield: 0, stun: 0, poison: 0, berserk: 0 });
  ...
  const doBossTurn = (hp, bhp, fx, cds) => {
    ...
    // Tick player buffs
    ["guard", "shield", "berserk"].forEach(k => { if (newFx[k] > 0) newFx[k]--; });
    ...
    const dmg     = calcBossDmg(fx); // use pre-tick buffs for calculation
  ...
  const SKILL_BORDER = { attack: "#7c3aed", defense: "#1d4ed8", heal: "#15803d", special: "#9333ea" };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: dark ? "#0a0a0a" : "#f9fafb", zIndex: 500, display: "flex", flexDirection: "column", fontFamily: "Inter, system-ui, sans-serif", overflow: "hidden" }}>
      ...
      <div style={{ padding: "1rem 1.5rem", borderBottom: `1px solid ${border}`, display: "flex", alignItems: "center", gap: "1rem", flexShrink: 0 }}>
        ...
        <div style={{ marginLeft: "auto", fontSize: "0.72rem", color: fgMuted }}>
          {turn === "player" && !outcome ? <span style={{ color: "#4ade80" }}>your turn</span> : null}
          {turn === "boss"   && !outcome ? <span style={{ color: "#ef4444" }}>boss turn</span>   : null}
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "1.25rem 1.5rem", gap: "1rem", overflowY: "auto" }}>
        ...
        <div style={{ textAlign: "center", padding: "0.5rem 0", position: "relative" }}>
          {effects.stun > 0  && <span style={{ position: "absolute", top: 0, left: "50%", transform: "translateX(-50%)", fontSize: "1.4rem" }}>💫</span>}
          {effects.poison > 0 && <span style={{ position: "absolute", top: 4, right: "38%", fontSize: "0.9rem" }}>☠️</span>}
        ...
        {turn === "player" && !outcome && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px" }}>
            {character.skills.map((skillId, i) => {
